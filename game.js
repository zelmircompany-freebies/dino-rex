...
class GameEngine {
  ...
  _drawPlayer(){
    ...
    if(UI.heroGifOk){
      // пользовательская гифка доступна: рисуем DOM <img> поверх canvas
      // вместо процедурного пиксель-спрайта, сохраняя тот же хитбокс/позицию.
      this._positionGameHeroGif(drawX, drawY, dims, p);
    } else {
  ...
  // позиционирует DOM-элемент #game-hero-gif точно поверх текущего кадра canvas,
  // повторяя размер/позицию процедурного спрайта и разворот при приседе/смерти
  _positionGameHeroGif(drawX, drawY, dims, p){
    const el = document.getElementById('game-hero-gif');
    if(!el) return;
    ...
    el.style.left = (offsetX + drawX) + 'px';
    el.style.top = (offsetY + drawY) + 'px';
    el.style.width = dims.w + 'px';
    el.style.height = dims.h + 'px';

    // лёгкий визуальный фидбек состояния: смерть -> притемняем и переворачиваем,
    // приседание -> слегка сплющиваем по вертикали
    let transform = '';
    if(p.state === 'dead'){ transform = 'scaleY(0.9) rotate(8deg)'; el.style.opacity = '0.85'; }
    else if(p.state === 'duck'){ transform = 'scaleY(0.75)'; el.style.opacity = '1'; }
...
const UI = {
  ...
  showScreen(id){
    ...
    if(id !== 'game'){
      const gameGif = document.getElementById('game-hero-gif');
      if(gameGif) gameGif.style.display = 'none';
    }
  ...
  /* ---------- Пользовательская гифка героя (img/dino.gif) ----------
     Файл кладётся пользователем в img/dino.gif рядом с игрой.
     Если файла нет — используем canvas-фолбэк (процедурный спрайт),
     чтобы меню не выглядело сломанным до того как гифка появится. */
  _setupHeroGif(){
    const img = document.getElementById('hero-gif');
    const gameImg = document.getElementById('game-hero-gif');
    const canvas = document.getElementById('hero-canvas');
    if(!img) return;

    this.heroGifOk = false;

    const showFallback = () => {
      img.classList.add('gif-unavailable');
      canvas.style.display = 'block';
      this.heroGifOk = false;
      if(gameImg) gameImg.style.display = 'none';
    };
    const showGif = () => {
      img.classList.remove('gif-unavailable');
    ...
    img.addEventListener('load', () => {
      if(img.naturalWidth > 0){ showGif(); } else { showFallback(); }
    });

    if(img.complete){
      if(img.naturalWidth > 0){ showGif(); } else { showFallback(); }
    } else {
      showFallback(); // по умолчанию фолбэк, пока загрузка не подтвердится
    }
  },
